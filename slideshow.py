...
class HeaderSlide(Slide):
    """Title of the service"""

    def __init__(self, presentation, infotext1, infotext2):
        super().__init__(presentation, True, indent=2400, vertindent=1700)
        self.cursor.ParaLeftMargin = 0
        self.InsertHeader(infotext1,'white', italics=False, center=True, customsize=40)
        self.InsertHeader(infotext2,'yellow', italics=True, customsize=40)
...
def LuoMessu(songs,evankeliumi=None):
    ...
    messu = Presentation()
    HeaderSlide(messu, songs['ylaots'], songs['alaots'])
    #Johdanto
...
def ExtractStructure(mailfile):
    ...
    #Get the next sunday and extract information accordingly from a csv file
    nextsu = GetNextSu()
    with open('/home/juho/Dropbox/srk/infot/kevat_2016.csv','r') as f:
        for row in csv.DictReader(f):
            row['Pvm'] = datetime.datetime.strptime(row['Pvm'], "%d.%m.%Y")
            if row['Pvm'].date() - nextsu == datetime.timedelta(0):
                break
    


    songs = dict()
    songs['ylaots'] = 'Majakkamessu {}'.format((row['Pvm'].strftime('%-d.%-m')))
    songs['alaots'] = row['Aihe']
    ...
    #match = re.search(r'evankeliumi: ?(.*)',row['Saarnateksti'].lower())
    match = re.search(r'(\d?\.?\w+) (\d+):([0-9,-]+)',row['Saarnateksti'].lower())
    book = match.group(1).replace(' ','')
    chapter = match.group(2)
    verse = match.group(3)
    evankeliumi = BibleText(book,chapter,verse,row['Saarnateksti'])
    ...
    for songrole, songname in songs.items():
        if songrole not in ('ylistyslaulut','ehtoollislaulut','ylaots','alaots'):
            songs[songrole] = CheckAvailability(songname)
        elif songrole not in ('ylaots','alaots'):
...
class BibleText:
    ...
    def GetBibleText(self, book, chapter, verse):
        ...
        book = self.BookNameToEnglish(book)
        text = ''
        if '-' in verse:
            verses = verse.split('-')
            start = int(verses[0])
    ...
    def BookNameToEnglish(self, book):
        with open('booknames.csv','r') as f:
            for row in csv.DictReader(f):
                if book == row['fi'].lower():
                    return row['en']
            return input('Anna englanninkielinen nimi kirjalle >>{}<<. Se ei löytynyt lyhenteiden listasta.'.format(book))
